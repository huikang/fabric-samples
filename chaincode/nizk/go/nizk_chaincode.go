...
// ==== Invoke marbles ====
// peer chaincode invoke -C myc1 -n marbles -c '{"Args":["initBank","marble1","blue","35","tom"]}'
// peer chaincode invoke -C myc1 -n marbles -c '{"Args":["initBank","marble2","red","50","tom"]}'
// peer chaincode invoke -C myc1 -n marbles -c '{"Args":["initBank","marble3","blue","70","tom"]}'
...
type bank struct {
	ObjectType string `json:"docType"` //docType is used to distinguish the various types of objects in state database
	TXID       int    `json:"txId"`    //the fieldtags are needed to keep case from bouncing around
	Asset      string `json:"asset"`
	Timestamp  string `json:"timestamp"`
	ID         int    `json:"Id"`
	PCommit    int    `json:"pcommit"`
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "initBank" { //create a new marble
...
// ============================================================
// initBank - create a new bank, store into chaincode state
// ============================================================
func (t *SimpleChaincode) initBank(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
...
// ===========================================================
// transfer a specific amount of an asset between two banks
// ===========================================================
func (t *SimpleChaincode) transferAsset(stub shim.ChaincodeStubInterface, args []string) pb.Response {

	//   0        1         2         3
	// "bank1", "bank2", "asset", "amount"
	if len(args) < 4 {
		return shim.Error("Incorrect number of arguments. Expecting 4")
	}

	bank1Name := strings.ToLower(args[0])
	bank2Name := strings.ToLower(args[1])
	assetName := strings.ToLower(args[2])
	amount, err := strconv.Atoi(args[3])
	if err != nil {
		return shim.Error("4th argument must be a numeric string")
	}
	//newOwner := strings.ToLower(args[1])
	fmt.Println("- start transfer ", amount, " ", asset, " from " bank1Name, " to ", bank2Name)

	//============================================
	// need to do the Proof of Asset at this step
	// todo!
	//============================================
	// marbleAsBytes, err := stub.GetState(marbleName)
	// if err != nil {
